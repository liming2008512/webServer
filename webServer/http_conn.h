...
public:
    ...
    /*行的读取状态，分别表示：读取到一个完整的行、行出错、行数据不完整*/
...
public:
    /*所有socket上的事件都被注册到同一个epoll内核事件表中，所以将epoll文件描述符设置为static*/
...
private:
    ...
    /*我们采用writev来执行写操作，所以定义下面两个成员，其中m_iv_count标识被写内存块的数量*/
    struct iovec m_iv[2];
    int m_iv_count;
};
